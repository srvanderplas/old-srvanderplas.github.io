...
gg2animint(viz)
```
```{r worldbankdemo2,eval=FALSE,echo=FALSE,include=FALSE}
data(WorldBank)
WorldBank <- subset(WorldBank, !is.na(life.expectancy) & !is.na(year) & !is.na(fertility.rate))
WorldBank$region <- factor(gsub(" (all income levels)", "", as.character(WorldBank$region), fixed=T))
viz <-
  list(ts=ggplot()+
...
tornado.bar <- list(map = map, ts = ts, 
                    width=list(map = 970, ts = 500),  
                    height=list(400)) 
# specify plot widths to be 970px and 500px respectively, 
...
gg2animint(tornado.anim, out.dir = "tornado-anim")
...
- [d3 graphics](http://bl.ocks.org/mbostock)

- [Today's Slides](https://srvanderplas.github.io/Presentations/Animint/animint.html#/)
=======
...
>>>>>>> cd51fe1d89be408b3ab2193523ac3a08b6f23d61
