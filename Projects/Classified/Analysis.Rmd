...
We can use our posts to generate new posts as well. Using n-grams (strings of $n$ words), we can generate new posts that are based on the conditional probability that "this" follows "that". Specifically, if $x$ is a string of $n$ words and $y$ is the $(n+1)$th word, we are modeling $P(y | x)$, that is, the probability that $y$ is next after $x$. So if we're modeling 2-grams, and the last 2 words we read were "Just in", we'd likely consider "case" to have high probability of being the next word. 

The `ngram` package splits our text up into ngrams and then recombines the ngrams to form an entirely new text using the `babble()` function. We'll use the functions in the ngram package to create new craigslist ads. 
...
# Fix punctuation, then collapse all posts into a single string
babble.text <- str_replace_all(posts$post.text, "http:\\\\[a-zA-Z1-9]{1,}\\.[a-zA-Z]{1,4}", " ")
babble.text <- str_replace_all(babble.text, "[\\()]", " ")
babble.text <- str_replace_all(babble.text, "([\\.,\\?!]{1,})", " \\1 ")
babble.text <- str_replace_all(babble.text, " ([\\.,\\?!]{1})", "\\1")
babble.text <- str_replace_all(babble.text, "\\n", " ")
babble.text <- tolower(babble.text)
babble.text <- str_replace_all(babble.text, "( ){2,}", " ")
...
print(ngram.res, output="truncated")
...
set.seed(240958208)
tmp <- babble(ngram.res, 1000)
# tmp
fix.babble(tmp)[1:2]
```

This sounds slightly more schizophrenic than the typical Craigslist ad, so let's ramp up our game and use 3-grams in our model. Conditioning on an extra word should give us more sentance continuity, so that the topic doesn't change back and forth quite so much. 
...
out.posts <- fix.babble(babble(ngram.res, 1000))
out.posts[1:3]
```

Better, but still quite prone to rambling and such. The originals are a bit better, at least. 
...
# Generate a new sequence of 1000 words
out.posts <- fix.babble(babble(ngram.res, 500))
out.posts[1:3]
```
