...
We can use our posts to generate new posts as well. Using n-grams (strings of $n$ words), we can generate new posts that are based on the conditional probability that "this" follows "that". 
...
# Generate a new ad with 200 words
babble(ngram.res, 200)
...
fix.babble <- function(text){
  gen.posts <- str_trim(str_split(text, "END BEGIN")[[1]])
...
fix.babble(tmp)[1:2]
```
Not awful, but we can do better. Let's repeat this using 3-grams. 
...
fix.babble(babble(ngram.res, 1000))[1:2]
```
