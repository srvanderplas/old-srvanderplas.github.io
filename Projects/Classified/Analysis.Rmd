...
# Use a stemmed DTM for this part
stem.corpus <- tm_map(myCorpus, stemDocument)
stem.dtm <- DocumentTermMatrix(stem.corpus)

# Remove sparse terms

dtm.common <- removeSparseTerms(stem.dtm, 0.95)

# Carson's steps (https://github.com/cpsievert/cpsievert.github.com/blob/master/projects/615/xkcd/fitLDA.R)

doc.id <- rep(dtm.common$i, dtm.common$v)
word.id <- rep(dtm.common$j, dtm.common$v)
n.iter <- 1000
ks <- seq(5, 50, by=5)
res <- NULL
ctr <- 1
for(i in ks){
  res[[ctr]] <- fitLDA(word.id, doc.id, k=i, n.chains=3, n.iter=n.iter, alpha=25/i, beta=0.1)
}

# Start with the 9 sub-boards for posting classifieds
model <- LDA(as.TermDocumentMatrix(dtm.common), k=9)
```